...
$range = explode(' - ', $_SESSION['search_topup_day_data']['s_time_range']);
...
while (1) {
    ...
    $day_formatted = date('d-m-Y', $day_stamp);
	...
	if($usd!=0)
	{
		$table_data_str .='<tr><td>'.$day_formatted.'</td><td>USD</td><td class="text-right">'.number_format($usd,2,".","").'</td></tr>';
	}
	if($gbp!=0)
	{
		$table_data_str .='<tr><td>'.$day_formatted.'</td><td>GBP</td><td class="text-right">'.number_format($gbp,2,".","").'</td></tr>';
	}
	if($eur!=0)
	{
		$table_data_str .='<tr><td>'.$day_formatted.'</td><td>EURO</td><td class="text-right">'.number_format($eur,2,".","").'</td></tr>';
	}
	if($inr!=0)
	{
		$table_data_str .='<tr><td>'.$day_formatted.'</td><td>INR</td><td class="text-right">'.number_format($inr,2,".","").'</td></tr>';
	}
	if($susd!=0)
	{
		$table_data_str .='<tr><td>'.$day_formatted.'</td><td>S USD</td><td class="text-right">'.number_format($susd,2,".","").'</td></tr>';
	}
    ?>
    <script>
        /*making final array
         ...
         */
        label_array["<?php echo $i; ?>"] = "<?php echo $day_formatted; ?>";
            ...
            <form class="block-content form-horizontal " id="search_form" name="search_form"  method="post" action="">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-5 col-sm-9 col-xs-12">
                        <input type="text" name="time_range" id="time_range" class="form-control " value="<?php if (isset($_SESSION['search_topup_day_data']['s_time_range'])) echo $_SESSION['search_topup_day_data']['s_time_range']; ?>" readonly="readonly" data-parsley-required="" />
                    </div>   
                            ...
                            <select name="account_manager" id="account_manager" class="form-control" tabindex="<?php echo $tab_index++; ?>">
                                ...
                                if (isset($ac_mngrs_data['result']) && count($ac_mngrs_data['result']) > 0) {
                                    foreach ($ac_mngrs_data['result'] as $key => $ac_mngr_array) {
                                        $selected = ' ';
                                        if ($ac_mngr_array['user_access_id_name'] == $_SESSION['search_topup_day_data']['s_account_manager'])
                    ...
                    <?php else:
                        ...
                        <div class="col-md-2 col-sm-9 col-xs-12">
                            <input type="text" class="form-control data-search-field" name="account_id" id="account_id" value="<?php echo $_SESSION['search_topup_day_data']['s_account_id']; ?>">
                        </div>
