...
$(document).ready(function() {
    ...
    var table = $('#datatable-buttons').DataTable( {
			...
			dataSrc : function (json) {
				...
				$.each(json.data, function(key,obj) {
					$.each(obj,function(k,value){
				  		 if(k == 'callstatus') {
						 	if(value == 'answer') countAnswer++;
							else if(value == 'ring') countRinging++;
							else if(value == 'progress') countProgress++;
						}
					});
				});
		...
		fnRowCallback: function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
			if( aData.callstatus == "ring" ) $('td', nRow).css({'background-color':'#F2A2A4','color':'#990000'}) ;
			else if( aData.callstatus == "answer" ) $('td', nRow).css({'background-color':'#AEFF93','color':'#006600'});
		},
		sDom: 'l<"toolbox">ftip',
		searching: true,
		paging: false,
	...
	setInterval( function () {
		...
		if(checked) {  console.log('Not loading');}
		else {console.log('Loading'); table.ajax.reload();}
	}, 5000 );	
} );
  </script>