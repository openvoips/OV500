 ...
 */
class Blockcli_mod  extends CI_Model{
	...
	{		
		...
		{	
			$sql = "SELECT SQL_CALC_FOUND_ROWS 						
					cli.id, cli.cli, cli.account_id, customers.company_name
					FROM ".$this->db->dbprefix('block_cli')."  cli
			...
			$final_return_array['message']='DND Numbers Fetched Successfully';
	...
	try{
			...
			$sql = "SELECT cli FROM ".$this->db->dbprefix('block_cli')." WHERE cli='".$param['cli']."' AND account_id='".$param['account_id']."'"; 
			$query = $this->db->query($sql);
			$row = $query->row_array();
			if(isset($row))
			{	
			...
			if(count($dnd_data_array)>0)			
			{
				...
				//echo '<pre>';print_r($dnd_data_array);echo '<pre>'; die;
			
				$str = $this->db->insert_string($this->db->dbprefix('block_cli'), $dnd_data_array); 
				//echo $str;die;
	...
	function delete($id_array)
	{
		try
		{
			...
			foreach($id_array['delete_id'] as $id)
			{
				...
				////delete user ///////
				$sql = "SELECT * FROM ".$this->db->dbprefix('block_cli')." WHERE id='".$id."' ";
				$query = $this->db->query($sql);
				$row = $query->row_array();								
				if(isset($row))
				{
					$data_dump = serialize($row);					
					$result = $this->db->delete($this->db->dbprefix('block_cli'), array('id' => $id));
					if(!$result) 
					{
						$error_array = $this->db->error();
						throw new Exception($error_array['message']);
	...
	function add_blockcli_number($account_id, $filename, $csv_data)
	{
		try
		{
			...
			if(count($dnd_data_array)>0)			
			{
				$result = $this->db->insert_batch($this->db->dbprefix('block_cli'), $final_dnd_data_array); 				
				if(!$result) 
				{
					$error_array = $this->db->error();
					throw new Exception($error_array['message']);
