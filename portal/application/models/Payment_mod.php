...
class Payment_mod extends CI_Model {
        ...
        try {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                if (isset($data['order_status']) && $data['order_status'] == 'success') {
                    ...
                    $api_request['SERVICENUMBER'] = 'Bank Transfer Payment';
                    $api_request['COLLECTIONOPTION'] = $data['collection_option'];
                    $api_request['AMMOUNT'] = $amount;
                    $api_request['PAIDON'] = date('Y-m-d H:i:s');
                    $api_request['NOTES'] = $payment_method_display . ' payment. Trans ID: (' . $data['tracking_id'] . ')' . ' (approved by ' . ucwords($approvedby) . ')';
    ...
    function add($data) {
        ...
        try {      
            ...
            $api_request['NOTES'] = $api_request['notes'];
            $api_request['CREATEDBY'] = get_logged_user_id();
    ...
    function payment_confirm($account_id, $user_type, $method, $data) {
        ...
        try {
            ...
            $api_request['ACCOUNTID'] = $account_id;
            $api_request['USERTYPE'] = $user_type;
            $api_request['SERVICENUMBER'] = 'Bank Transfer Payment';
            $api_request['COLLECTIONOPTION'] = 'Bank Transfer Payment';
            $api_request['AMMOUNT'] = $payment_data_array['transactions'][0]['amount']['total'];
            $api_request['PAIDON'] = date('Y-m-d H:i:s');
