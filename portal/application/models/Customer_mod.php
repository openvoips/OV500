...
//            if (isset($option_param['CAMPAINGN']) && $option_param['CAMPAINGN'] == true && count($final_return_array['result']) > 0) {
//                $account_id_str = implode("','", $account_id_array);
//                $account_id_str = "'" . $account_id_str . "'";
//                $sql = "SELECT * FROM campaigns WHERE account_id IN($account_id_str) ";
//                if (isset($option_param['dst_campaign_id'])) {
//                    $sql .= " AND campaign_id ='" . $option_param['dst_campaign_id'] . "'";
//                }
//                $query = $this->db->query($sql);
//                if (!$query) {
//                    $error_array = $this->db->error();
...
//                    $account_id = $row['account_id'];
//                    $id = $row['id'];
//                    $final_return_array['result'][$account_id]['CAMPAINGN'][$id] = $row;
//                }
//            }
            ...
            if (isset($option_param['sipuser']) && $option_param['sipuser'] == true && count($final_return_array['result']) > 0) {
                $account_id_str = implode("','", $account_id_array);
                $account_id_str = "'" . $account_id_str . "'";
                $sql = "SELECT * FROM customer_devices  WHERE user_type = 'SWITCH' and  account_id IN($account_id_str) ";
            ...
            if ((isset($option_param['bundle_package']) || isset($option_param['bundle_package_group_by'])) && count($final_return_array['result']) > 0) {
                ...
                $sql = "SELECT *, (select GROUP_CONCAT(prefix) from bundle_package_prefixes where  bundle_package_prefixes.bundle_package_id = customer_bundle_sdr.bundle_package_id   and prefix <> '') prefix, customer_bundle_sdr.id bundle_account_id, count(customer_bundle_sdr.bundle_package_id) bundle_count FROM customer_bundle_sdr INNER JOIN bundle_package ON customer_bundle_sdr.bundle_package_id = bundle_package.bundle_package_id WHERE customer_bundle_sdr.account_id IN  ($account_id_str)";
                ...
                if (isset($option_param['bundle_package_group_by'])) {
                    $sql .= " GROUP BY customer_bundle_sdr.bundle_package_id";
                }
