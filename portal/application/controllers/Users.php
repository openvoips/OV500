...
class Users extends CI_Controller {
    ...
    function index($arg1 = '', $format = '') {
        ...
        if (isset($_POST['search_action'])) {
            $_SESSION['search_users_data'] = array(
                's_name' => $_POST['name'],
                's_username' => $_POST['username'],
                's_status' => $_POST['status'],
                's_account_type' => $_POST['account_type'],
                's_account_id' => $_POST['account_id'],
                's_no_of_records' => $_POST['no_of_rows'],
            );
        } else {
            $_SESSION['search_users_data']['s_name'] = isset($_SESSION['search_users_data']['s_name']) ? $_SESSION['search_users_data']['s_name'] : '';
            $_SESSION['search_users_data']['s_username'] = isset($_SESSION['search_users_data']['s_username']) ? $_SESSION['search_users_data']['s_username'] : '';
            $_SESSION['search_users_data']['s_status'] = isset($_SESSION['search_users_data']['s_status']) ? $_SESSION['search_users_data']['s_status'] : '';
            $_SESSION['search_users_data']['s_account_type'] = isset($_SESSION['search_users_data']['s_account_type']) ? $_SESSION['search_users_data']['s_account_type'] : '';
            $_SESSION['search_users_data']['s_account_id'] = isset($_SESSION['search_users_data']['s_account_id']) ? $_SESSION['search_users_data']['s_account_id'] : '';
            $_SESSION['search_users_data']['s_no_of_records'] = isset($_SESSION['search_users_data']['s_no_of_records']) ? $_SESSION['search_users_data']['s_no_of_records'] : RECORDS_PER_PAGE;
        }


        $search_data = array('name' => $_SESSION['search_users_data']['s_name'], 'username' => $_SESSION['search_users_data']['s_username'], 'account_status' => $_SESSION['search_users_data']['s_status'], 'account_type' => $_SESSION['search_users_data']['s_account_type'], 'account_id' => $_SESSION['search_users_data']['s_account_id']);
        ...
        if ($arg1 == 'export' && $format != '') {
            ...
            $users_data = $this->member_mod->get_data($order_by, $per_page, $segment, $search_data);
            $total = $this->member_mod->get_data_total_count();

            //prepare search data
            $search_array = array();
            if ($_SESSION['search_users_data']['s_account_id'] != '')
                $search_array['Account ID'] = $_SESSION['search_users_data']['s_account_id'];
            if ($_SESSION['search_users_data']['s_name'] != '')
                $search_array['Name'] = $_SESSION['search_users_data']['s_name'];
            if ($_SESSION['search_users_data']['s_username'] != '')
                $search_array['Username'] = $_SESSION['search_users_data']['s_username'];
            if ($_SESSION['search_users_data']['s_account_type'] != '')
                $search_array['User Type'] = $_SESSION['search_users_data']['s_account_type'];
            if ($_SESSION['search_users_data']['s_status'] != '')
                $search_array['Status'] = $_SESSION['search_users_data']['s_status'] == 1 ? 'Active' : 'Inactive';


            // column titles
            $export_header = array('Account ID', 'Name', 'Username', 'User Type', 'Status');

            if (count($users_data['result']) > 0) {
                foreach ($users_data['result'] as $account_data) {
                    ...
                    $account_type_name = $account_type_array[$account_type];

                    $export_data[] = array($account_data['account_id'], $account_data['name'], $account_data['username'], $account_type_name, $status);
                }
            } else
                $export_data = array('');
        ...
        if ($is_file_downloaded === false) {
            ...
            $account_group_key = 1; //admin
            $data['account_type_array'] = get_account_types($account_group_key);
            /*             * **** pagination code start here ********* */
            $pagination_uri_segment = 3;
            if (isset($_SESSION['search_users_data']['s_no_of_records']) && $_SESSION['search_users_data']['s_no_of_records'] != '')
                $per_page = $_SESSION['search_users_data']['s_no_of_records'];
            else
            ...
            $config = array();
            $config = $this->utils_model->setup_pagination_option($total, 'users/index', $per_page, $pagination_uri_segment);
    ...
    public function add_admin() /* Add Admin User */ {
        $page_name = "account_add_admin";
        $data['page_name'] = $page_name;
        ...
        if (isset($_POST['action']) && $_POST['action'] == 'OkSaveData') {
            ...
            if ($this->form_validation->run() == FALSE) {// error
                ...
                if ($result === true) {
                    $account_id = $this->member_mod->account_id; //new inserted user id									
                    ...
                    if (isset($_POST['button_action']) && trim($_POST['button_action']) != '') {
                        ...
                        if ($action == 'save')
                            redirect(base_url() . 'users/add_admin', 'location', '301');
        ...
        $data['account_type_array'] = get_account_types($account_group_key);
        $this->load->view('basic/header', $data);
        $this->load->view('user/admin_add', $data);
        $this->load->view('basic/footer', $data);
    }

    public function edit_admin() /* Edit Admin User */ {
        $page_name = "account_edit_admin";
        $data['page_name'] = $page_name;
        ...
        if (isset($_POST['action']) && $_POST['action'] == 'OkSaveData') {//print_r($_POST);// die;
            ...
            if ($this->form_validation->run() == FALSE) {// error
                ...
                if ($result === true) {
                    ...
                    if ($this->member_mod->account_access_id != '' && $this->member_mod->account_access_id == $this->session->userdata('session_current_account_id')) {
                    ...
                    if (isset($_POST['button_action']) && trim($_POST['button_action']) != '') {
                        $action = trim($_POST['button_action']);
                        if ($action == 'save')
                            redirect(base_url() . 'users/edit_admin/' . param_encrypt($account_id), 'location', '301');
                        elseif ($action == 'save_close')
                            redirect(base_url() . 'users', 'location', '301');
                    }
                    else {
        ...
        if (!empty($users_id_p)) {
            $users_id = param_decrypt($users_id_p);

            ////
            $order_by = '';
            $per_page = 1;
            $segment = 0;
            $search_data = array('admin_id' => $users_id);
            $users_data_temp = $this->member_mod->get_data($order_by, $per_page, $segment, $search_data);
            //	echo '<pre>';	echo $users_id;print_r($users_data_temp);die;
            ...
            $users_data = arrayToObject($users_data);
        } else {//die("DD");
            show_404();
        }
        /*         * **** pagination code ends  here ********* */
    ...
    public function switch_user($id) /**/ {
        ...
        $userdata = array('session_current_account_id' => $account_id);
    ...
    public function unswitch_user($id) /**/ {
        ...
        unset($_SESSION['users'][$account_id]);
    ...
    public function autologin($id, $login_as = 'self') /**/ {
        ...
        $data['page_name'] = $page_name;
        if (!check_logged_account_type(array('ADMIN', 'SUBADMIN', 'RESELLER', 'NOC'))) {
            show_404('403');
        }
        $account_id = param_decrypt($id);
        $result = $this->member_mod->get_account_by_key('account_access_id_name', $account_id);
        if ($result === false)
            show_404();
        ...
        if ($login_as == 'self') {
            ...
            $parent_result = $this->member_mod->get_account_by_key('account_access_id_name', $result['parent_account_id']);
        ...
        if (!$row) {
            ...
            $userdata = array();
            $userdata = array('session_current_account_id' => $row->account_id);
            ...
            $this->session->set_userdata($userdata);
            $_SESSION['users'][$row->account_id] = $userdata_details;

            $this->session->set_flashdata('suc_msgs', 'You are switched to ' . get_account_full_name());

            if (isset($child_account_type)) {
                if (in_array($child_account_type, array('CUSTOMER'))) {
                    $redirect_to = 'endusers';
                    unset($_SESSION['search_endusers_data']);
                    $_SESSION['search_endusers_data']['s_account_id'] = $result['account_id'];
                ...
                } elseif (in_array($child_account_type, array('CARRIER'))) {
                    $redirect_to = 'carriers';
                    unset($_SESSION['search_carrier_data']);
                    $_SESSION['search_carrier_data']['s_carrier_id_name'] = $result['account_id'];
                }
            }
        }
