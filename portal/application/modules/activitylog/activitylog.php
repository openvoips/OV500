...
* Author: Tonmoy Tewary
* Author URI: http://openvoips.org/tonmoy
*/
?>
<?php
// 'plugin_'.$directory_name.'_install'
function plugin_activitylog_install()
{
	$sql = "CREATE TABLE  IF NOT EXISTS `webaccess_log` (
...
{
	$sql = 'DROP TABLE IF EXISTS `webaccess_log`';
	$ci =& get_instance();
	$query = $ci->db->query($sql);
	if (!$query) {
		$error_array = $ci->db->error();
