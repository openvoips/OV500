...
class Clifilter extends MY_Controller
{
	...
	public function index($account_id_temp = -1, $type = 'did')
	{
		...
		////////////////////////////////////
		//
		$search_parameters = array('callerid', 'cli_status', 'no_of_rows');
		...
		$search_data = array(
			...
			'cli_status' => $_SESSION[$search_session_key]['cli_status'],
			'callerid' => $_SESSION[$search_session_key]['callerid'],
		);
		...
		if ($is_file_downloaded === false) {
			...
			$cli_data = $this->clifilter_mod->get_cli_data($type,'', $per_page, $segment, $search_data);
			
			$total_count = $this->clifilter_mod->get_data_total_count();
			$data['pagination'] = setup_pagination_option($total_count, 'crs/clifilter/index/' . $account_id_temp . '/' . $type, $per_page, $pagination_uri_segment, $this->pagination);
