<div style="margin-bottom:75px;"> 
                                ...
                                <tbody>
                                                ...
                                                if (count($data['randomcli']) > 0) {
                                                    foreach ($data['randomcli'] as $randomcli_data) {
                                                        if ($randomcli_data['rule_type'] != '2')
                                                            continue;
                                                        if ($randomcli_data['cli_status'] == '1')
                                                            $status = '<span class="label label-success">Active</span>';
                                                        else
                                                            $status = '<span class="label label-danger">Inactive</span>';
                                                        $delete_link = "crs/customers/randomclis/" . param_encrypt($randomcli_data['account_id']) . '/2';
                                                        $str .= '<tr>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12 text-right">
                                <a href="<?php echo site_url('crs/customers/add_randomcli/' . param_encrypt($data['account_id']) . '/2'); ?>"><input
