...
function dispay_pagination_row_bsd($total_records, $s_no_of_records, $pagination) {
    ...
    $str .= '</div>
            <div class=" col-md-3 col-sm-12 col-xs-12 page_records">
...
function aa_create_menu_html_bsd($menu_array, $page_name) {
        ...
        if (isset($sub_menu_array['menu_name'])) {//single page link
            ...
            foreach ($sub_menu_array as $sub_key => $sub_sub_menu_array) {
                ...
                if (isset($sub_menu_array[$menu_name]) && count($sub_menu_array[$menu_name]) > 0) {
                    ...
                    $menu_temp .= '<li ' . $class_sub . '><a href="' . $link_url . '"  ' . $target . '>' . $icon_sub . '<span>' . $menu_name . '</span></a></li>';
                }
            }
            if ($menu_temp != '') {
...
function mybalance() {
    ...
    if ($logged_customer_account_id != 'SYSTEM') {
        $sql = "SELECT concat( round(if (customer_balance.balance = 0, 0, -1 * customer_balance.balance ), 2), ' ',account.currency_id) balance FROM `customer_balance` INNER JOIN account on account.account_id = customer_balance.account_id
where customer_balance.account_id = '" . $logged_customer_account_id . "' ;";
...
function differenceBetweenTwoTimeZone($regionArea1, $regionArea2) {
    $currentTimeInZone1 = new DateTime("now", new DateTimeZone($regionArea1));
    $currentTimeDiff1 = $currentTimeInZone1->format('Z');

    $currentTimeInZone2 = new DateTime("now", new DateTimeZone($regionArea2));
...
function get_user_types($key = '') {
    ...
    $user_type_array[3] = array('CUSTOMERADMIN' => 'Customer Admin', 'CUSTOMERUSER' => 'Customer User', 'EXTENSION' => 'Extension User');
...
function get_logged_data() {
    ...
    $session_user_id_name = $_SESSION['customer'][$session_current_user_id];
    foreach ($session_user_id_name as $key => $value) {
        $key_new = str_replace('session_', '', $key);
        $data[$key_new] = trim($value);
    }
    return $data;
}
...
function get_permission_options($user_type) {
    ...
    if ($user_group_key == ADMIN_ACCOUNT_ID) {
        $permission_array = array(
            ...
            'bundle' => array('view', 'add', 'edit', 'delete'),
            'vendor' => array('view', 'add', 'edit', 'delete'),
            'service' => array('view', 'add', 'edit', 'delete'),
            'system' => array('system_load'),
            'payment_gateway' => array('add'),
    ...
    } elseif ($user_group_key == 'RESELLER' || $user_group_key == 'RESELLERADMIN') {
        $permission_array = array(
            'user' => array('view', 'add', 'edit', 'login'),
            'reseller' => array('view', 'add', 'edit', 'delete', 'cliedit', 'login'),
            'customer' => array('view', 'add', 'edit', 'delete', 'cliedit', 'login'),
...
function check_account_permission($item_name, $permission_name = '') {
    $is_permitted = false; //change_march_2023
...
function generate_key($name, $type = '') {
    ...
    $abbr = strtoupper($abbreviate($name, 4));
...
function set_activity_log($log_data_array) {//$activity_type, $sql_table, $sql_key,$sql_query,$logged_customer_id
    if (!is_array($log_data_array) || count($log_data_array) == 0)
        return;
    $CI = & get_instance();
...
function send_mail($message, $subject, $mail_to, $mail_from, $mail_from_name, $cc = '', $bcc = '', $account_id = '', $actionfrom = '', $attachment_array = array(), $smpt_details = Array()) {

 //   print_r($smpt_details);
    ...
    $mail_from = $smpt_details['smtp_from_email'];
    $mail_from_name = $smpt_details['smtp_from_name'];
    /////
    if (count($smpt_details) > 0) {
        $mail_config['protocol'] = 'smtp';
        $mail_config['mailpath'] = '/usr/sbin/sendmail';
        $mail_config['smtp_host'] = $smpt_details['smtp_host'];
        $mail_config['smtp_user'] = $smpt_details['smtp_user'];
        $mail_config['smtp_pass'] = $smpt_details['smtp_pass'];
        $mail_config['smtp_port'] = $smpt_details['smtp_port'];
        $mail_config['smtp_crypto'] = $smpt_details['smtp_secure'] = "tls";
        $mail_from = $smpt_details['smtp_from_email'];
        $mail_from_name = $smpt_details['smtp_from_name'];
    }
...
function setup_pagination_option($total, $page, $per_page, $segment, &$pagination_instance) {
    ...
    $config['full_tag_open'] = '<ul class="pagination pagination-sm justify-content-end" style="margin:6px 0;">';
    $config['full_tag_close'] = '</ul>';
    $config['num_tag_open'] = '<li>';
    $config['num_tag_close'] = '</li>';
    $config['cur_tag_open'] = '<li class="page-item active"><a href="javascript:void(0);" class="current"><b>';
    $config['cur_tag_close'] = '</b></a></li>';
    $config['prev_link'] = '<i class="success fa fa-angle-double-left"></i> ';
    $config['prev_tag_open'] = '<li class="previous">';
    $config['prev_tag_close'] = '</li>';
    $config['next_link'] = ' <i class="success fa fa-angle-double-right"></i>';
...
function get_currency($currency_id, $format = 'name') {
    ...
    $currency_array = array(
        '1' => array('name' => 'ZAR', 'symbol' => '<i class="fa fa-usd"></i>'), //'&#36;'
        '2' => array('name' => 'GBP', 'symbol' => '<i class="fa fa-gbp"></i>'), //'&#163;'
        '3' => array('name' => 'INR', 'symbol' => '<i class="fa fa-inr"></i>'), //'&#128;'
        '4' => array('name' => 'USD', 'symbol' => '<i class="fa fa-usd"></i>'), //'&#36;'
        '5' => array('name' => 'EURO', 'symbol' => '<i class="fa fa-eur"></i>'), //'&#x20B9;'
    );
...
//////////////////////
    ...
    $menu_array[$menu_title]['reports/CRecharge'] = array(
        'page_name' => array('CRecharge'),
        'page_url' => 'reports/CRecharge',
        'menu_name' => 'Topup Customer Summary'
    );
...
/////////////
    ...
    $menu_array[$menu_title]['sysconfig/pGConfig'] = array(
        'page_name' => array('payment_config'),
        'page_url' => 'sysconfig/pGConfig',
        'menu_name' => 'Payment Gateway Config'
    );
...
function encrypt($string) {
    ...
    for ($i = 0; $i < strlen($string); $i++) {
        ...
        $keychar = substr("OV500BASICSYSTEMOWNBYOPENVOIPS@GMAIL.COM", ($i % strlen("OV500BASICSYSTEMOWNBYOPENVOIPS@GMAIL.COM")) - 1, 1);
